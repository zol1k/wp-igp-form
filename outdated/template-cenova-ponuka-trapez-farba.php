<?php
/**
 * Template Name: Cenová ponuka - Trapez - Farba
 *
 */
?>
<?php get_header() ?>
        ...
        <div class="container">
            <div class="tw-max-w-[1200px] tw-mx-auto">
                ...
                <div class="tw-hidden lg:tw-flex tw-items-center tw-justify-center tw-mb-40 tw-gap-8">
                    ...
                    <!-- Krok 2 - Dokončený -->
                    <div class="tw-flex tw-items-center tw-cursor-pointer hover:tw-opacity-80 tw-transition-opacity" onclick="window.location.href='/angebot/trapezbleche-typ/'">
                        <div class="tw-w-32 tw-h-32 tw-rounded-full tw-bg-primary-900 tw-flex tw-items-center tw-justify-center tw-text-white tw-font-bold tw-text-[14px]">
                            <svg class="tw-w-18 tw-h-18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            </svg>
                ...
                <div class="tw-mb-20 md:tw-mb-50">
                    ...
                    if ($items && is_array($items)) {
                        ...
                        foreach ($items as $item) {
                            ...
                            if (!$location || !$colors || !$subitems) continue;
                            foreach ($colors as $color_post) {
                                ...
                                foreach ($subitems as $sub) {
                                    ...
                                    <div style="display:none;" class="color-card tw-relative tw-bg-white tw-border-2 tw-rounded-8 md:tw-rounded-12 tw-p-6 md:tw-p-10 lg:tw-p-12 tw-cursor-pointer hover:tw-border-primary-900 tw-transition-all tw-flex tw-flex-col tw-items-center tw-min-h-[120px] md:tw-min-h-[140px] lg:tw-min-h-[160px] tw-border-neutral-200"
                                        ...
                                        data-location-type="<?php echo esc_attr($location . '-' . $type); ?>"
                                            ...
                                            <div style="border: 1px solid #e6e6e6; border-radius: 7px;background-image: url('<?php echo esc_attr(get_the_post_thumbnail_url($color_post->ID, 'medium')); ?>'); background-size: cover; background-position: center;" class="tw-w-full tw-aspect-square tw-rounded-8 tw-mb-4 tw-overflow-hidden">
                                                ...
                                                <div class="tw-flex tw-flex-col tw-items-center tw-justify-center tw-h-full">
                                                    <?php foreach ($thickness as $thick) : ?>
                                                        <span style="text-shadow: #000 1px 1px 4px;" class="tw-text-white tw-font-bold tw-text-[12px] md:tw-text-[14px] lg:tw-text-[16px] tw-px-2 tw-py-1 tw-bg-black/50 tw-rounded-4 tw-mb-1">[<?php echo esc_html($thick); ?> mm]</span>
                                                    <?php endforeach; ?>
                                                </div>
                                            </div>
...
function filterColors() {
    ...
    let trapezType = sessionStorage.getItem('trapezType');
    let trapezSubtype = sessionStorage.getItem('trapezSubtype');

    let mergedTypeAndSubtypeInLowercase = normalizeSlug(trapezType) + '-' + normalizeSlug(trapezSubtype);
    //console.log('Merged type and subtype:', mergedTypeAndSubtypeInLowercase);
    ...
    cards.forEach(card => {
        ...
        const locationInLowercase = normalizeSlug(card.getAttribute('data-location-type'));
        //console.log('Card location:', locationInLowercase);
        //console.log('Merged type and subtype for comparison:', mergedTypeAndSubtypeInLowercase);
        if (locationInLowercase === mergedTypeAndSubtypeInLowercase) {
            show = true;
        }
...
function selectColor(element) {
    ...
    sessionStorage.setItem('trapezColor', color);
}
...
function handleFileSelect(input) {
    ...
    if (input.files[0]) {
        sessionStorage.setItem('trapezFileName', fileName);
        console.log('session stored trapezFileName:', fileName);
        document.getElementById('removeFile').classList.remove('tw-hidden');
    } else {
        sessionStorage.removeItem('trapezFileName');
...
function saveTextarea() {
    ...
    sessionStorage.setItem('trapezInfo', textarea.value);
    console.log('session stored trapezInfo:', textarea.value);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Restore checkbox
    const savedOdkapovySystem = sessionStorage.getItem('trapezOdkapovySystem');
    ...
    // Restore color
    const savedColor = sessionStorage.getItem('trapezColor');
    if (savedColor) {
        console.log("Restoring color selection:", savedColor);
        // select card with matching data-color attribute and that is not hidden, there is more of them but only one should be visible due to filtering
        const card = Array.from(document.querySelectorAll('.color-card')).find(c => c.getAttribute('data-color') === savedColor && c.style.display !== 'none');
        if (card) {
            console.log("Card found for saved color:", card);
            selectColor(card);
        }
    }

    // Restore textarea
    const savedInfo = sessionStorage.getItem('trapezInfo');
    if (savedInfo) {
        document.getElementById('dodatocneInfo').value = savedInfo;
    }

    // Restore file name
    const savedFileName = sessionStorage.getItem('trapezFileName');
    if (savedFileName) {
...
function toggleOdkapovySystem() {
    ...
    sessionStorage.setItem('trapezOdkapovySystem', checkbox.checked);
    console.log('Odkapový systém checkbox state changed:', checkbox.checked);
}

async function validateAndSubmit() {
    const color = sessionStorage.getItem('trapezColor');
    if (!color) {
    ...
    formData.append('action', 'send_trapez_quote');
    ...
    formData.append('selectedProductName', sessionStorage.getItem('selectedProductName') || 'trapez');
    
    // Údaje o streche
    formData.append('type', sessionStorage.getItem('trapezTitle') || '');
    formData.append('subType', sessionStorage.getItem('trapezSubtypeTitle') || '');
    formData.append('color', sessionStorage.getItem('trapezColor') || '');
    
    // Farba a doplnky
    formData.append('odkapovySystem', sessionStorage.getItem('trapezOdkapovySystem') === 'true' ? 'Yes' : 'No');
    formData.append('info', sessionStorage.getItem('trapezInfo') || '');
    formData.append('file', sessionStorage.getItem('trapezFileName') || 'Nie');
    ...
    try {
        ...
        if (result.success) {
            ...
            window.location.href = '/trapezbleche-danke/';
        } else {
...
function goBack() {
    window.location.href = '/angebot/trapezbleche-typ/';
}
</script>
