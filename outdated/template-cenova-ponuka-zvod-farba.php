<?php
/**
 * Template Name: Cenová ponuka - Zvod - Farba
 *
 */
?>
<?php get_header() ?>
    ...
    <section class="tw-pt-[100px] md:tw-pt-[120px] lg:tw-pt-[150px] tw-pb-24 md:tw-pb-50">
        <div class="container">
            <div class="tw-max-w-[1200px] tw-mx-auto">
                ...
                <!-- Progress stepper - Mobile Version -->
                <div class="tw-block lg:tw-hidden tw-mb-20 md:tw-mb-40">
                    <div class="tw-text-center tw-mb-12">
                        <div class="tw-text-[12px] md:tw-text-[14px] tw-text-neutral-600 tw-mb-4" style="display:none;">Krok 3 z 3</div>
                        <div class="tw-text-[16px] md:tw-text-[20px] tw-font-bold tw-text-primary-900">Farbe</div>
                    </div>
                ...
                <!-- Progress stepper - Desktop Version -->
                <div class="tw-hidden lg:tw-flex tw-items-center tw-justify-center tw-mb-40 tw-gap-8">
                    ...
                    <!-- Krok 2 - Dokončený -->
                    <div class="tw-flex tw-items-center tw-cursor-pointer hover:tw-opacity-80 tw-transition-opacity" onclick="window.location.href='/angebot/dachrinnensystem-typ/'">
                        <div class="tw-w-32 tw-h-32 tw-rounded-full tw-bg-primary-900 tw-flex tw-items-center tw-justify-center tw-text-white tw-font-bold tw-text-[14px]">
                            <svg class="tw-w-18 tw-h-18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path>
                            </svg>
                ...
                <div class="tw-mb-20 md:tw-mb-50">
                    ...
                    <div id="color-error" class="tw-hidden tw-text-error-500 tw-text-[14px] md:tw-text-[16px] tw-mt-8 tw-font-medium tw-px-8 tw-py-4 tw-rounded-8 tw-bg-error-500/10">Sie müssen die Farbe des Entwässerungssystems auswählen</div>
                </div>
...
function selectColor(element) {
    ...
    sessionStorage.setItem('zvodColor', color);
    console.log('session stored zvodColor:', color);
}
...
function handleFileSelect(input) {
    ...
    if (input.files[0]) {
        sessionStorage.setItem('zvodFileName', fileName);
        console.log('session stored zvodFileName:', fileName);
        document.getElementById('removeFile').classList.remove('tw-hidden');
    } else {
        sessionStorage.removeItem('zvodFileName');
...
function saveTextarea() {
    ...
    sessionStorage.setItem('zvodInfo', textarea.value);
    console.log('session stored zvodInfo:', textarea.value);
}

document.addEventListener('DOMContentLoaded', function() {
    // Restore color
    const savedColor = sessionStorage.getItem('zvodColor');
    if (savedColor) {
    ...
    // Restore textarea
    const savedInfo = sessionStorage.getItem('zvodInfo');
    if (savedInfo) {
        document.getElementById('dodatocneInfo').value = savedInfo;
    }

    // Restore file name
    const savedFileName = sessionStorage.getItem('zvodFileName');
    if (savedFileName) {
...
async function validateAndSubmit() {
    const color = sessionStorage.getItem('zvodColor');
    if (!color) {
    ...
    formData.append('action', 'send_zvod_quote');
    ...
    // Zvod špecifikácia
    formData.append('zvodVersion', sessionStorage.getItem('zvodVersion') || '');
    formData.append('zvodColor', sessionStorage.getItem('zvodColor') || '');
    formData.append('zvodInfo', sessionStorage.getItem('zvodInfo') || '');
    formData.append('zvodFileName', sessionStorage.getItem('zvodFileName') || '');
    ...
    try {
        ...
        if (result.success) {
            ...
            window.location.href = '/dachrinnensystem-danke/';
        } else {
...
function goBack() {
    window.location.href = '/angebot/dachrinnensystem-typ/';
}
</script>
